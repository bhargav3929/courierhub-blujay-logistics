...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!verifyWebhook(rawBody, hmacHeader, SHOPIFY_GAYATRI_API_SECRET)) {
            console.error('[Shopify-Gayatri Webhook] Invalid HMAC signature');
            return NextResponse.json({ error: 'Invalid signature' }, { status: 401 });
        }

        const payload = JSON.parse(rawBody);

        console.log(`[Shopify-Gayatri Webhook] Received ${topic} from ${shopDomain}`);
        ...
        if (topic === 'orders/create') {
            ...
            if (snapshot.empty) {
                console.log('[Shopify-Gayatri Webhook] No user found for shop:', shopDomain);
            ...
            // Verify this is a gayatri app user
            if (userData.shopifyConfig?.appId !== 'gayatri') {
                console.log('[Shopify-Gayatri Webhook] User not connected via gayatri app');
                return NextResponse.json({ received: true });
            }
            ...
            console.log('[Shopify-Gayatri Webhook] Created shipment for order:', order.id);
        }

        return NextResponse.json({ received: true });

    } catch (error: any) {
        console.error('[Shopify-Gayatri Webhook] Error:', error);
