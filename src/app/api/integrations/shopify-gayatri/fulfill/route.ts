...
export async function POST(request: Request) {
    ...
    try {
        ...
        // Verify this user is using the gayatri app
        if (shopifyConfig.appId !== 'gayatri') {
            return NextResponse.json({ error: 'User is not connected via gayatri app' }, { status: 400 });
        }

        const accessToken = decryptTokenWithSecret(shopifyConfig.accessToken, SHOPIFY_GAYATRI_API_SECRET);
        ...
        if (!fulfillmentOrdersResponse.ok) {
            ...
            console.error('[Shopify-Gayatri Fulfill] Failed to get fulfillment orders:', errorText);
        ...
        if (!fulfillResponse.ok) {
            ...
            console.error('[Shopify-Gayatri Fulfill] Failed to create fulfillment:', errorText);
        ...
        console.error('[Shopify-Gayatri Fulfill] Error:', error);
