...
async function findUserByPendingShop(shopDomain: string): Promise<string | null> {
    try {
        ...
        console.error('[Shopify-Gayatri Callback] Error looking up user by shop:', error);
        return null;
    }
}
...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (!SHOPIFY_GAYATRI_API_SECRET || !SHOPIFY_GAYATRI_API_KEY) {
        ...
        if (state) {
            userId = verifySignedState(state, SHOPIFY_GAYATRI_API_SECRET);
        }

        if (!userId) {
            console.log('[Shopify-Gayatri Callback] No valid state, looking up user by shop domain:', shop);
        ...
        try {
            const accessTokenResponse = await fetch(`https://${shop}/admin/oauth/access_token`, {
                ...
                body: JSON.stringify({
                    client_id: SHOPIFY_GAYATRI_API_KEY,
                    client_secret: SHOPIFY_GAYATRI_API_SECRET,
                    code,
                }),
            });
            ...
            if (!accessTokenResponse.ok) {
                ...
                console.error('[Shopify-Gayatri Callback] Token exchange failed:', accessTokenResponse.status, errorText);
            ...
            console.error('[Shopify-Gayatri Callback] Token exchange error:', error);
            return NextResponse.redirect(`${APP_URL}/client-integrations?shopifyError=token_exchange_failed`);
        }

        // 4. If no userId — store as pending install
        if (!userId) {
            console.log('[Shopify-Gayatri Callback] No userId found — storing as pending install for shop:', shop);
            try {
                await setDoc(doc(db, 'pendingShopifyInstalls', shop), {
                    accessToken: encryptTokenWithSecret(accessToken, SHOPIFY_GAYATRI_API_SECRET),
                    scopes: tokenScopes,
                ...
                console.error('[Shopify-Gayatri Callback] Failed to store pending install:', e);
            }
        ...
        try {
            ...
            const webhookResult = await registerShopifyWebhook(
                shop, accessToken, '/api/integrations/shopify-gayatri/webhook'
            );
            if (!webhookResult.success) {
                console.error('[Shopify-Gayatri Callback] Webhook registration failed:', webhookResult.error);
                await updateDoc(doc(db, 'users', userId), {
            ...
            console.error('[Shopify-Gayatri Callback] Error:', error);
            return NextResponse.redirect(`${APP_URL}/client-integrations?shopifyError=callback_failed`);
        }

    } catch (error: any) {
        console.error('[Shopify-Gayatri Callback] Unhandled error:', error);
        return NextResponse.redirect(`${APP_URL}/client-integrations?shopifyError=callback_failed`);
    }
}
