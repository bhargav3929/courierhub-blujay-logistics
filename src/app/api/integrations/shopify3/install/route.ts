...
export async function GET(request: Request) {
    try {
        ...
        if (pendingDoc.exists() && !pendingDoc.data().claimed) {
            console.log('[Shopify3 Install] Found pending install for', shopUrl, '— claiming for user', userId);

            const pendingData = pendingDoc.data();
            const accessToken = decryptTokenWithSecret(pendingData.accessToken, SHOPIFY3_API_SECRET);
            ...
            const webhookResult = await registerShopifyWebhook(shopUrl, accessToken, '/api/integrations/shopify3/webhook');
            if (!webhookResult.success) {
                console.error('[Shopify3 Install] Webhook registration failed:', webhookResult.error);
                await updateDoc(doc(db, 'users', userId), {
            ...
            console.log('[Shopify3 Install] Pending install claimed successfully for user', userId);
        ...
        const scopes = 'read_orders,write_fulfillments,read_products,read_merchant_managed_fulfillment_orders,write_merchant_managed_fulfillment_orders';
