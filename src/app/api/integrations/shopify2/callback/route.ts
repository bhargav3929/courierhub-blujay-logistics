...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (state) {
            userId = verifySignedState(state, SHOPIFY2_API_SECRET);
        }
        ...
        try {
            ...
            if (!accessTokenResponse.ok) {
                ...
                console.error('[Shopify2 Callback] Token exchange failed:', accessTokenResponse.status, errorText);
        ...
        if (!userId) {
            console.log('[Shopify2 Callback] No userId found — storing as pending install for shop:', shop);
            try {
                ...
                console.error('[Shopify2 Callback] Failed to store pending install:', e);
            }
        ...
        console.error('[Shopify2 Callback] Unhandled error:', error);
        return NextResponse.redirect(`${APP_URL}/client-integrations?shopifyError=callback_failed`);
    }
}
