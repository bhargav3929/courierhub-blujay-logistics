...
export async function POST(request: Request) {
    try {
        ...
        if (!hmac || !shopDomain || !SHOPIFY2_API_SECRET) {
        ...
        if (topic === 'app/uninstalled') {
            console.log(`[Shopify2 Webhook] APP_UNINSTALLED for ${shopDomain}`);
            ...
            for (const userDocument of uSnap.docs) {
                ...
                console.log(`[Shopify2 Webhook] Marked ${shopDomain} disconnected for user ${userDocument.id}`);
            }
        ...
        if (querySnapshot.empty) {
            console.error(`[Shopify2 Webhook] Received webhook for unknown shop: ${shopDomain}`);
        ...
        console.log(`[Shopify2 Webhook] Processing Order #${order.order_number} for User: ${userId}`);
        ...
        if (!existingShipments.empty) {
            console.log(`[Shopify2 Webhook] Duplicate webhook: Order #${order.order_number} already processed`);
        ...
        // Fetch defaultPickupAddress from the clients collection
        ...
        try {
            ...
            console.error('[Shopify2 Webhook] Failed to fetch client defaultPickupAddress:', err);
        }
        ...
        console.log(`[Shopify2 Webhook] Shipment created for Order #${order.order_number}`);

        return NextResponse.json({ message: 'Webhook processed successfully' });

    } catch (error: any) {
        console.error('[Shopify2 Webhook] Error:', error);
