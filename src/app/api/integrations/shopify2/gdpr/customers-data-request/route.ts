...
/**
 * GDPR: customers/data_request (App 2)
...
export async function POST(request: Request) {
    try {
        ...
        console.log(`[GDPR App2] customers/data_request from ${shop_domain} for customer ${customer?.email}`);
        ...
        console.log(`[GDPR App2] Stored data request with ${shipmentResults.length} shipments`);

        return NextResponse.json({ received: true });

    } catch (error: unknown) {
        console.error('[GDPR App2] customers/data_request error:', error);
        return NextResponse.json({ received: true });
    }
}
