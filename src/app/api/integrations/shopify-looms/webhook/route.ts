...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!verifyWebhook(rawBody, hmacHeader, SHOPIFY_LOOMS_API_SECRET)) {
            console.error('[Shopify-Looms Webhook] Invalid HMAC signature');
        ...
        if (topic === 'orders/create') {
            ...
            if (snapshot.empty) {
                console.log('[Shopify-Looms Webhook] No user found for shop:', shopDomain);
                return NextResponse.json({ received: true });
            }
            ...
            // Verify this is a looms app user
            if (userData.shopifyConfig?.appId !== 'looms') {
                console.log('[Shopify-Looms Webhook] User not connected via looms app');
                return NextResponse.json({ received: true });
            }
            ...
            console.log('[Shopify-Looms Webhook] Created shipment for order:', order.id);
        }

        return NextResponse.json({ received: true });

    } catch (error: any) {
        console.error('[Shopify-Looms Webhook] Error:', error);
