...
export async function POST(request: Request) {
    ...
    try {
        ...
        // Verify this user is using the looms app
        if (shopifyConfig.appId !== 'looms') {
            return NextResponse.json({ error: 'User is not connected via looms app' }, { status: 400 });
        }

        const accessToken = decryptTokenWithSecret(shopifyConfig.accessToken, SHOPIFY_LOOMS_API_SECRET);
        ...
        if (!fulfillmentOrdersResponse.ok) {
            ...
            console.error('[Shopify-Looms Fulfill] Failed to get fulfillment orders:', errorText);
        ...
        if (!fulfillResponse.ok) {
            ...
            console.error('[Shopify-Looms Fulfill] Failed to create fulfillment:', errorText);
        ...
        console.error('[Shopify-Looms Fulfill] Error:', error);
