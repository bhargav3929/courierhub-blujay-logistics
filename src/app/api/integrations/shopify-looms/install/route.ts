...
export async function GET(request: Request) {
    try {
        ...
        console.log('[Shopify-Looms Install] shop:', shop, 'userId:', userId, 'API_KEY exists:', !!SHOPIFY_LOOMS_API_KEY);
        ...
        if (!SHOPIFY_LOOMS_API_KEY || !SHOPIFY_LOOMS_API_SECRET) {
            console.error('[Shopify-Looms Install] Missing env vars - API_KEY:', !!SHOPIFY_LOOMS_API_KEY, 'API_SECRET:', !!SHOPIFY_LOOMS_API_SECRET);
        ...
        if (pendingDoc.exists() && !pendingDoc.data().claimed && pendingDoc.data().appId === 'looms') {
            console.log('[Shopify-Looms Install] Found pending install for', shopUrl, '— claiming for user', userId);

            const pendingData = pendingDoc.data();
            const accessToken = decryptTokenWithSecret(pendingData.accessToken, SHOPIFY_LOOMS_API_SECRET);
            ...
            // Register webhook
            const webhookResult = await registerShopifyWebhook(shopUrl, accessToken, '/api/integrations/shopify-looms/webhook');
            if (!webhookResult.success) {
                console.error('[Shopify-Looms Install] Webhook registration failed:', webhookResult.error);
                await updateDoc(doc(db, 'users', userId), {
            ...
            console.log('[Shopify-Looms Install] Pending install claimed successfully for user', userId);
        ...
        const redirectUri = `${APP_URL}/api/integrations/shopify-looms/callback`;
        ...
        const installUrl = `https://${shopUrl}/admin/oauth/authorize?client_id=${SHOPIFY_LOOMS_API_KEY}&scope=${scopes}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${state}`;

        console.log('[Shopify-Looms Install] Redirecting to OAuth URL');

        return NextResponse.redirect(installUrl, { status: 302 });
    } catch (error: any) {
        console.error('[Shopify-Looms Install] Error:', error);
